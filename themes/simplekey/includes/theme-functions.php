		...
		foreach($posts as $post){
          ...
          if($thumbnail<>1){
            ...
            if(has_post_thumbnail()){
              $recent_blog.='<div class="thumbnail"><a href="'.$url.'" title="'.$title.'"><img src="'.$thumbnail_url[0].'" /></a></div>';
			}
		...
		foreach($posts as $post){
			...
			if($showimg==1){ 
				if(has_post_thumbnail($post->ID)){
					  ...
					  $post_list.='<a href="'.$url.'" class="thumbnail"><img src="'.$thumbnail_url[0].'" /></a>';
				}
		...
		$post_slider_html.='<div id="post-gallery" class="flexslider">
            <ul class="slides maxHeight">';
              foreach( $images as $image ) {
                ...
                $post_slider_html.='<li><img src="'.$attachmenturl.'" /></li>';
                }
             $post_slider_html.='</ul>
           </div>';
	    }
		...
		foreach($posts as $post){
		   ...
		   if(has_post_thumbnail($post->ID)){
			...
			$on_slug = join( " ", $slug);
				...
				if($intro==1){
				  $return_html.='<p class="intro">'.$excerpt.'</p>';
				}
				$return_html.='</a>
				<div class="tools">';
